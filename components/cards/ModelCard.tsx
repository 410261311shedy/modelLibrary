...
const ModelCard = ({selectedCategory}:ModelCardProps) => {
  

  // filter card based on selectedCategory
  const filteredCard = selectedCategory ==="ALL" ? card : card.filter((item)=>(item.category === selectedCategory)) 

  return (
    <>
      {filteredCard.map((card)=>(
        <div key={card.id} onClick={(e)=>{return console.log("Card pressed", e);}} className='hover-lift cursor-pointer w-[400px] sm:w-[250px] md:w-[25vw] h-[300px]'>
            ...
            <div className=" mt-2 px-5 flex items-center justify-between text-sm min-w-0">
              <p className='flex items-center gap-1 min-w-0'>
                <Image
                  ...
                  height={16}
                />
                <span className='truncate text-black dark:text-[#E4E4E7] font-medium'>{card.title}</span>
              </p>
