...
### 09:45 | PDF 閱讀器功能完善與截圖整合
- **SSR 相容性處理**：
    - 將 `PDFViewer` 拆分為 `PDFViewer.tsx` (Entry) 與 `PDFViewerInternal.tsx` (Implementation)。
    - 使用 `next/dynamic` 配合 `ssr: false` 載入閱讀器，徹底解決 `pdfjs-dist` 在伺服器端找不到 `canvas` 模組的編譯錯誤。
- **資料流與記憶體優化**：
    - 實作 `File` 物件自動轉換為 `Blob URL` 的邏輯，支援本地檔案即時預覽。
    - 在 `useEffect` 清理函數中加入 `URL.revokeObjectURL`，確保在切換檔案或組件卸載時釋放記憶體，防止大型 PDF 造成的瀏覽器崩潰。
- **封面擷取系統 (Screenshot) 實作**：
    - 整合 `modern-screenshot` 庫，利用 `domToCanvas` 擷取 PDF 渲染容器。
    - 透過 `forwardRef` 與 `useImperativeHandle` 將 `takeScreenshot` 方法暴露給父組件。
    - 更新 `app/(uploadAndDashboard)/upload/page.tsx`：在 Step 2 點擊 "Next" 時，根據檔案類型（3D 或 PDF）自動呼叫對應的截圖介面，並將結果存入 `coverImage`。
- **監聽react-pdf-viewer中的活動避免在canvas未重新渲染完成時讓使用者進行下一個操做避免連續操作造成的黑屏**：
